...
//        unset($_SESSION['nyanmalan']);
            
        if ($this->skapa_deltagare() == TRUE){
            $this->extract_post();
...
//            unset $_POST;
    ...
    private function save_driver() {
        ...
        if ($this->driver->id == -1){   // om ny förare ska sparas
            $this->create_new_driver($user_array);
        } elseif ($this->driver->id > 0) {
    ...
    private function check_password() {
        // Kollar om password är samma i båda fälten
...
//         return TRUE; // Sätter $driver->new_driver till TRUE om password inte ska uppdateras  
    ...
    public function driver_data($id) {
        ...
        $value=$this->driver->{$row->user_data_descr}->value;
                $row->value = $value;
            }
        }
        return $user_data;
    ...
    private function extract_post() {
        ...
        if(!isset($_POST['use_driver'])){
            $this->driver->use_driver = $user->id();
        }else{
    ...
    private function extract_post_key(){
        foreach ($_POST['key'] as $index=>$key){    
            ...
            $user_data['value'] = isset($_POST['value'][$index])?$_POST['value'][$index]:NULL;
    ...
    private function skapa_deltagare(){
        global $user;
        $logged_in = $user->logged_in();
        $nyanmälan = isset($_SESSION['nyanmalan'])?TRUE:FALSE;
        if(isset($_POST['save'])){
            return TRUE;
            ...
            $_POST['use_driver'] = $user->logged_in()? $_SESSION['user']: -1;
//            $_POST['use_driver'] = -1;
            $return = TRUE;
            return $return;
        }
